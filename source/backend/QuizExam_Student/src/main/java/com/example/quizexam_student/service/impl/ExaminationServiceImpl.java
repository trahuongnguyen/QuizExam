...
public class ExaminationServiceImpl implements ExaminationService {
    ...
    @Override
    public Examination createAutoGeneratedExam(ExaminationRequest examinationRequest) {
        ...
        List<Examination> examinations = examinationRepository.findTop3BySubjectAndTypeOrderByIdDesc(subject, 0);
    ...
    @Override
    public Examination createExamFromSelectedQuestions(ExaminationRequest examinationRequest, List<Integer> questionIds) {
        if (examinationRequest.getEndTime().isBefore(examinationRequest.getStartTime())) {
            throw new InvalidTimeException("endTime", "End time must be after start time");
        }
        Subject subject = subjectRepository.findById(examinationRequest.getSubjectId()).orElse(null);
    ...
    @Override
    public Examination updateExamQuestion(int examinationId, List<Integer> questionIds) {
        Examination examination = examinationRepository.findByIdAndStatus(examinationId,1);
        Examination newExamination = new Examination();
        assert examination != null;
        examination.setStatus(2);
        examinationRepository.save(examination);
        newExamination.setName(examination.getName());
        newExamination.setStartTime(examination.getStartTime());
        newExamination.setEndTime(examination.getEndTime());
        newExamination.setDuration(examination.getDuration());
        newExamination.setSubject(examination.getSubject());
        newExamination.setType(examination.getType());
        newExamination.setStatus(1);
        double maxScore = 0;
        List<Question> questions = questionRepository.findAllByIdInAndStatus(questionIds, 1);
        for (Question question : questions) {
            generateAnswerForQuestion(question);
        ...
        newExamination.setMaxScore(maxScore);
        newExamination.setTotalQuestion(questions.size());
        newExamination.setQuestions(new HashSet<>(questions));
        newExamination.setCode(examination.getCode());
        for (Question question : questions) {
            setAnswerForQuestion(newExamination, question);
        }
        newExamination = examinationRepository.save(newExamination);
        List<Mark> marks = markRepository.findAllByExamination(examination);
        Examination finalNewExamination = newExamination;
        marks.forEach(mark -> {
            mark.setExamination(finalNewExamination);
            markRepository.save(mark);
        });
        return finalNewExamination;
    }
